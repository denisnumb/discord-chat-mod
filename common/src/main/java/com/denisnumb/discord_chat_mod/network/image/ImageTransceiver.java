...
public class ImageTransceiver {
        ...
        networkPool.execute(() -> {
            try {
                ...
                BigPacketsTransceiver.send(data, (partIndex, totalParts, part) ->
                        PlatformPacketDistributor.sendToServer(new ImagePartPacketServer(currentTime, partIndex, totalParts, part))
                );
            } catch (Exception e) {
    ...
    public static void receivePartClientSide(ImagePartPacketClient packet) {
        receivePartCommon(packet, ImageTransceiver::handleReceivedImageClient);
    }

    public static void receivePartServerSide(ImagePartPacketServer packet, ServerPlayer fromPlayer) {
        ...
        networkPool.execute(() -> {
            try {
                ...
                for (ServerPlayer player : allRecipients) {
                    try {
                        BigPacketsTransceiver.send(data, (partIndex, totalParts, part) ->
                                PlatformPacketDistributor.sendToPlayer(player, new ImagePartPacketClient(currentTime, partIndex, totalParts, part))
                        );
