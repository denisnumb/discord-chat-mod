...
public class ImageStorage {
    ...
    private static void releaseImageResources(AbstractImage image) {
        ...
        Minecraft.getInstance().execute(() -> {
            ...
            if (image instanceof AnimatedImage animated) {
                for (ResourceLocation frame : animated.frames) {
            ...
            if (image.spoilerResourceLocation != null) {
                if (tm.getTexture(image.spoilerResourceLocation) instanceof DynamicTexture dt)
                    dt.close();
                tm.release(image.spoilerResourceLocation);
            }
        });
    }

    private static void registerTexture(ResourceLocation location, NativeImage image) throws InterruptedException {
    ...
    private static ResourceLocation buildSpoilerTexture(String imageUrl, NativeImage image) throws InterruptedException {
